...
func add_slab_to_deck(slab_data: Dictionary):
	...
	# Optional: Shuffle deck immediately or keep it appended? 
	# Usually roguelikes shuffle only when the draw pile is empty, 
...
func add_artifact(artifact_id: String):
	if not artifacts.has(artifact_id):
		...
		print("Artifact added: ", artifact_id)
		# Here you would trigger immediate effects if the artifact has one on pickup

# --- Weight/RNG Manipulation ---
func increase_number_weight(number: int, weight_increase: int):
	# Initialize if not present
	if not number_bias.has(number):
		number_bias[number] = 0
	
	number_bias[number] += weight_increase
	print("Increased weight for number ", number, " by ", weight_increase)

# Update generate_grid_numbers to actually USE these weights
func get_weighted_random_number() -> int:
	var pool = []
	# Base weight of 10 for every number
	for i in range(1, 16):
