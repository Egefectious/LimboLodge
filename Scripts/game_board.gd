...
func update_ui():
	...
	draw_button.disabled = Global.draws_remaining <= 0 or (not current_slab.is_empty() and must_place_or_bench)
	bench_button.disabled = current_slab.is_empty() or not must_place_or_bench or Global.benched_slabs.size() >= Global.max_bench_slots
...
func create_slab_visual(parent: Node, slab: Dictionary, size: Vector2):
